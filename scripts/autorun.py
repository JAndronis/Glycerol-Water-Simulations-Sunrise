...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pressure", type=int, default=1, required=True, help="Reference pressure"
    )
    parser.add_argument(
        "--equilibration-time",
        type=int,
    ...
    output_dir = pathlib.Path(
        args.output_path, f"{run_prefix}_{current_time}"
    ).resolve()
    log = pathlib.Path(output_dir, "run.log")
    output_dir.mkdir()
    log.touch()

    passed_mdp_dir = pathlib.Path(args.mdp).resolve()
    ...
    # Launch jobs
    if args.launch:
        scripts = pathlib.Path(root, "scripts").resolve()
        if not args.gpu:
